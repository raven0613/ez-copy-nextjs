...
interface ITextCard {
    data: IUnit
    handleSave: (data: { id: string, value: string, tagList: string[], color: {text: string, bg: string}, createdAt: string, userId: string }) => void;
...
export default function TextCard({ data, handleSave, handleDelete, handleClick, handleEdit, idEditing, isExist, allTags }: ITextCard) {
    ...
    return (
        <div
            className={`${styles.textCard} ${false && styles.textCard__exsit}`}
            onClick={() => {
                handleClick(data.id);
                setIsClicked(true);
            }}
            ...
            <div className={styles.textCard__control}>
                ...
                <div className={styles.textCard__buttons}>
                    {idEditing && <button
                        ...
                        onClick={e => {
                            e.stopPropagation();
                            handleSave({ id: data.id, value: inputValue, tagList, color: {text: "", bg: ""}, createdAt: data.createdAt, userId: data.userId });
                        }}
                    >V
                    </button>}
                    {!idEditing && <button
